...
elseif (strpos($_SERVER['REQUEST_URI'], '/login') !== false){
    $authController = new AuthController(); // Crear una instancia del controlador de auth
    ...
    switch ($method) {
        case 'POST':
            // Para crear un nuevo grupo, se obtienen los datos del cuerpo de la solicitud
            $data = json_decode(file_get_contents("php://input"));
            $result = $authController->login($data); // Llamar al método create del controlador
            View::render($result); // Renderizar la respuesta
            break;
...
//         $result = $controller->auth($db, $data->email, $data->Pass);
